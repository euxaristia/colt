...
actor Main
  ...
  new create(env: Env) =>
    ...
    // Connect stdin
    env.input(recover iso StdinNotify(term) end, 512)
...
class StdinNotify is InputNotify
  """Forwards raw stdin bytes to ANSITerm for key interpretation."""
  let _t: ANSITerm tag

  new iso create(t: ANSITerm tag) =>
    _t = t
